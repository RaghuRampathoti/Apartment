...
const Toast = ({ toast, onClose }) => {
  ...
  return (
    <div className={`flex items-center gap-4 p-4 rounded-2xl shadow-2xl border transition-all duration-300 animate-in slide-in-from-right  ${toast.type === 'success'
        ? 'bg-emerald-50 border-emerald-100 text-emerald-800'
        : 'bg-rose-50 border-rose-100 text-rose-800'
      } min-w-[320px]`}>
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-sm ${toast.type === 'success' ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'
        }`}>
        {toast.type === 'success' ? <CheckIcon /> : <XIcon />}
      </div>
      <div className="flex-1">
        <div className="text-sm font-black uppercase tracking-widest leading-none mb-1">{toast.title}</div>
        <div className="text-xs font-bold opacity-80 leading-tight">{toast.message}</div>
      </div>
      <button className="w-6 h-6 rounded-lg hover:bg-black/5 flex items-center justify-center transition-colors" onClick={() => onClose(toast.id)}>✕</button>
    </div>
  );
};
...
export default function AdminDashboard() {
  ...
  const validateNoticeForm = () => {
    ...
    if (noticeFormData.type === 'EVENT' && !noticeFormData.eventDate) {
      errors.eventDate = "Event timing is required";
    }
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-50 text-slate-800 font-sans selection:bg-primary/10 selection:text-primary">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                      <div className="space-y-6">
                        <div className="group">
                          ...
                          {isEditingProfile ? (
                            <input type="text" className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-primary outline-none transition-all" value={profileFormData.username} onChange={(e) => setProfileFormData({ ...profileFormData, username: e.target.value.replace(/[^a-zA-Z\s]/g, '') })} />
                          ) : (
                      ...
                      <div className="space-y-6">
                        <div className="group">
                          ...
                          {isEditingProfile ? (
                            <input type="tel" maxLength="10" className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-primary outline-none transition-all" value={profileFormData.phone} onChange={(e) => setProfileFormData({ ...profileFormData, phone: e.target.value.replace(/\D/g, '').slice(0, 10) })} />
                          ) : (
                    ...
                    <div className="space-y-8">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="relative">
                            <input
                              type="file"
                              className={`w-full px-5 py-3.5 rounded-2xl border border-slate-200 bg-slate-50 text-xs font-bold text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-black file:bg-primary-light file:text-primary hover:file:bg-primary-light transition-all cursor-pointer ${noticeFile ? 'pr-14' : ''}`}
                            ...
                            {noticeFile && (
                              <button 
                                type="button"
                                className="absolute right-3 top-1/2 -translate-y-1/2 p-2 text-slate-400 hover:text-primary hover:bg-primary-light rounded-full transition-all"
                                onClick={() => {
                                  setViewImage(URL.createObjectURL(noticeFile));
                                  setViewFileType(noticeFile.type);
                                }}
                                title="Preview File"
                              >
                                <EyeIcon size={20} />
                      ...
                      {noticeFormData.type === 'EVENT' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-primary-light/50 rounded-3xl border border-primary/20 animate-in fade-in zoom-in-95 duration-300">
                          <div className="space-y-2">
                            <label className="text-[10px] font-black text-primary uppercase tracking-widest">Event Timing <span className="text-rose-500">*</span></label>
                            <input type="datetime-local" className={`w-full px-5 py-4 rounded-2xl border ${noticeErrors.eventDate ? 'border-rose-400 bg-rose-50' : 'border-primary/30 bg-white'} focus:ring-4 focus:ring-primary/10 outline-none transition-all font-bold text-slate-700`} value={noticeFormData.eventDate} onChange={(e) => {
                              setNoticeFormData({ ...noticeFormData, eventDate: e.target.value });
                              if (noticeErrors.eventDate) setNoticeErrors({ ...noticeErrors, eventDate: '' });
                            }} />
                            {noticeErrors.eventDate && <p className="text-xs font-bold text-rose-500 flex items-center gap-1 mt-2"><span><IoAlertCircleOutline size={14} /></span> {noticeErrors.eventDate}</p>}
                          </div>
                          <div className="space-y-2">
                    ...
                    <div className="p-10">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          ...
                          <input
                            type="text"
                            className={`w-full px-5 py-4 rounded-2xl border ${staffErrors.username ? 'border-rose-400 bg-rose-50' : 'border-slate-200 bg-slate-50'} focus:ring-4 focus:ring-primary/10 focus:bg-white outline-none transition-all font-bold text-slate-700`}
                            ...
                            onChange={(e) => {
                              ...
                              setStaffFormData({ ...staffFormData, username: lettersOnly });
                            }}
                          />
                          {staffErrors.username && <p className="text-[10px] font-black text-rose-500">{staffErrors.username}</p>}
                        </div>
                        <div className="space-y-2">
                        ...
                        <div className="space-y-2">
                          ...
                          <input
                            type="tel"
                            className="w-full px-5 py-4 rounded-2xl border border-slate-200 bg-slate-50 focus:ring-4 focus:ring-primary/10 focus:bg-white outline-none transition-all font-bold text-slate-700"
                            ...
                            onChange={(e) => {
                              ...
                              setStaffFormData({ ...staffFormData, phone: digits });
                            }}
                          />
                        </div>
                        <div className="space-y-2">
      ...
      {viewImage && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-8 bg-sidebar/80 backdrop-blur-md animate-in fade-in duration-300" onClick={() => { setViewImage(null); setViewFileType(null); }}>
          <div className="relative max-w-5xl w-full bg-white rounded-[2rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-4 bg-slate-50 flex items-center justify-center min-h-[400px]">
              {viewFileType === 'application/pdf' || viewImage.toLowerCase().endsWith('.pdf') ? (
                <iframe src={viewImage} className="w-full h-[80vh] rounded-2xl border border-slate-200" title="Document Preview" />
              ) : (
